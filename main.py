...
# follow_user
#
def follow_user(baseurl, token):
  """
  Follow another user based on their username
  """
  try:
    if not token:
      print("No current token, please log in")
    ...
    # Get username of user to follow
    print("Enter username of the user you want to follow:")
    username = input()
    ...
    # Prepare data for request - just send the username
    data = {
      "username": username
    ...
    print(f"Sending follow request for username: {username}...")
    res = requests.post(url, json=data, headers=headers)
    
    # Process response
    if res.status_code == 201:  # Created
      body = res.json()
      print("Success! You are now following this user.")
    ...
    logging.error("**ERROR: follow_user() failed:")
    logging.error(f"url: {url}")
    logging.error(e)
    return

def unfollow(baseurl, token):
  """
  ...
  try:
    ...
    # Get username of user to unfollow
    print("Enter username of the user you want to unfollow:")
    username = input()
    ...
    if res.status_code == 200:  # Success
      ...
      print(f"You are not currently following {username} or the user doesn't exist.")
    ...
    logging.error("**ERROR: unfollow() failed:")
    logging.error(f"url: {url}")
    logging.error(e)
    return
...
# create a post
# 
def create_post(baseurl, token):
    """
    Creates a new post with a caption and Spotify song
    """
    try:
        if not token:
            print("No current token, please log in")
        ...
        print("Enter your post caption (or press ENTER for no caption):")
        caption = input()
        
        # Get song information
        print("Enter song name to attach to your post: ")
        song_query = input()
        
        if not song_query.strip():
            print("Please attach a song. Try again")
            return
        
        ## psot
        # track id (song name for user) (this requries spotify authentication to get track ID)
        # caption
        ...
        logging.error("**ERROR: create_post() failed:")
        logging.error(f"url: {url}")
        logging.error(e)
        return
